...
% Get the current date and time formatted as desired
timestamp = datestr(now, 'yyyy-mm-dd HH:MM:SS');

fid = fopen(filename, 'wt'); % open a text file that we can write into

fprintf(fid, '%s % s\n',';',file_title);
fprintf(fid, '; File generated by the atom Toolbox in MATLAB at %s\n', timestamp);
fprintf(fid, '%s % s\n',';',strcat("Structure with ",num2str(size(atom,2))," atoms, ",num2str(size(Bond_index,1)),", bonds and ",num2str(size(Angle_index,1))," angles"));
fprintf(fid, '\n');
fprintf(fid, '%s\n','[ moleculetype ]');
fprintf(fid, '%s % s\n',';','molname   nrexcl');
