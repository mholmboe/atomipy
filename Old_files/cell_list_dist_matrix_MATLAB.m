...
function [dist_matrix,bond_list,dist_list,X_dist,Y_dist,Z_dist] = cell_list_dist_matrix_MATLAB(atom, Box_dim,varargin)
...
dist_matrix = single(zeros(N, N));
X_dist   = single(zeros(N, N));
Y_dist   = single(zeros(N, N));
Z_dist   = single(zeros(N, N));
...
dist_matrix(abs(dist_matrix) <= 1E-7) = 0;
dist_list(abs(dist_list) <= 1E-7) = 0;
X_dist(abs(X_dist) <= 1E-7) = 0;
Y_dist(abs(Y_dist) <= 1E-7) = 0;
Z_dist(abs(Z_dist) <= 1E-7) = 0;
