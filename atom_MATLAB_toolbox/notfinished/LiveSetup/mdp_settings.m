function mdp = mdp_settings(varargin)
...
%	Command line:
%	  gmx grompp -f file.mdp -c file.gro -r file.gro -p topol.top -n index.ndx -o file.tpr -pp -po
...
mdp.coulombtype              = 'PME'				;
mdp.coulomb_modifier         = 'Potential-shift-Verlet'					;
...
mdp.vdw_modifier             = 'Potential-shift-Verlet'					;
% cut_off lengths
mdp.rvdw_switch              = 0					;
mdp.rvdw                     = 1.0					;
...
if numel(sim)>0
    %% Overwrite the default mdp struct with the sim struct
    names = fieldnames(sim);
    for i=1:length(names)
        eval(['[mdp.',names{i},']=sim.',names{i},';']);
    end
end
mdp.dt

%% Write the .mdp file
fid=fopen('temp.mdp','w');
names = fieldnames(mdp);
% names=strrep(names,'_','_')
for i=1:length(names)
    eval([names{i} '=mdp.' names{i} ';']);
    fprintf(fid,'%-25s\t%s %s %\r\n',strrep(names{i},'_','-'),'=',num2str(eval(names{i})));
    fprintf(fid, '\r\n');
end
fprintf(fid, '\r\n');
fclose(fid);
